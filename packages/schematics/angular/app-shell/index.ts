...
function addAppShellConfig(options: AppShellOptions): Rule {
  return (host: Tree) => {
    ...
    if (!app) {
      throw new SchematicsException(`Clieng app (${options.clientApp}) could not be found.`);
    }
...
function addRouterOutlet(options: AppShellOptions): Rule {
  return (host: Tree) => {
    const routerOutletMarkup = `<routerOutlet></routerOutlet>`;
    ...
    if (templateProp) {
      if (!/<routerOutlet>/.test(templateProp.initializer.getText())) {
...
function addServerRoutes(options: AppShellOptions): Rule {
  return (host: Tree) => {
    ...
    if (!isImported(moduleSource, 'Routes', '@angular/router')) {
      ...
      const routeText =
        `\n\nconst routes: Routes = [ { path: '${options.route}', component: ShellComponent }];`;
